...
public class DispatcherServlet1 extends HttpServlet {
	...
	private void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("/guest_main.do")) {
			...
			String guest_noStr = request.getParameter("guest_no"); // 포워딩을 했기 때문에 173번라인의 리퀘스트와 같은 리퀘스트다
			if(guest_noStr==null||guest_noStr.equals("")){
				...
				try {
					...
					forwardPath="forward:/WEB-INF/views/guest_view.jsp"; //modify_action.do 에서 포워딩이 이뤄져서 최종나오는건 guest_view.jsp이다.
					/*
				if(guest==null){
					out.println("<script>");
